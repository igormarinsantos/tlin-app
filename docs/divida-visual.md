...
- `app/javascript/dashboard/components-next/sidebar/SidebarGroup.vue` e `SidebarGroupLeaf.vue`: o item ativo usa `bg-n-alpha-2`; para aplicar o gradiente apenas ao item ativo seria necessário editar as classes condicionais do core. O token de gradiente está disponível, mas não foi aplicado por seletor global.
