...
- `app/javascript/dashboard/components-next/sidebar/SidebarGroupLeaf.vue`, `SidebarGroupHeader.vue` e `SidebarGroup.vue`: exceção cirúrgica à regra de tema: o estado ativo recebeu `bg-tlin-gradient`, `rounded-full` e `text-n-black`, tanto nos links filhos quanto nos itens principais e no modo recolhido. Outros estados e a estrutura do componente core permanecem intactos.
