...
- `app/javascript/dashboard/components-next/sidebar/SidebarGroupLeaf.vue`, `SidebarGroupHeader.vue`, `SidebarGroup.vue` e `Sidebar.vue`: exceção cirúrgica à regra de tema: o estado ativo recebeu `bg-tlin-gradient`, `rounded-full` e `text-n-black`, tanto nos links filhos quanto nos itens principais e no modo recolhido. Os itens mantêm `rounded-full` também no hover e a navegação expandida recebeu espaçamento lateral. Outros estados e a estrutura do componente core permanecem intactos.
