...
> Decisão fork vs merge: FECHADA com dados — ver docs/plano/medicao-fork-vs-merge.md (merge é impossível: repo antigo não tem histórico compartilhado).
...
4. Seção E) SPEC DA ENGINE EXISTENTE: mapeie todo o código de IA em enterprise/lib/tlin/ e lib/tlin/ — liste cada tool, prompt e service com 2-3 linhas descrevendo comportamento, entradas e saídas. Isso vira a especificação funcional para reconstruirmos a engine limpa na base nova (referência de comportamento; o código em si NÃO será copiado por estar em pasta de licença comercial).
